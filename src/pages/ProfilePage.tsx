...
import { Flame, BookOpen, Star, Award, Settings, LogOut, Trophy, CheckCircle2, SlidersHorizontal } from "lucide-react";
...
export default function ProfilePage({ isGuest = false, onLogout }: ProfilePageProps) {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(syncGoals, 1000);
    window.addEventListener("storage", syncGoals);
    window.addEventListener("romingo:daily-goals-updated", syncGoals);
    window.addEventListener(learningEconomyUpdatedEvent, syncGoals);

    return () => {
      clearInterval(interval);
      window.removeEventListener("storage", syncGoals);
      window.removeEventListener("romingo:daily-goals-updated", syncGoals);
      window.removeEventListener(learningEconomyUpdatedEvent, syncGoals);
    };
  }, []);
